...
export default function App() {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter((t) => {
      ...
      const matchCiudad =
        filters.ciudad === "Todas las Ciudades" || t.ciudad === filters.ciudad;
  ...
  const zoneSales = useMemo<ZoneData[]>(() => {
    ...
    const total = Array.from(zoneMap.values()).reduce(
      (sum, value) => sum + value,
      0,
    );
  ...
  const paymentDistribution = useMemo<PaymentData[]>(() => {
    const paymentMap = new Map<string, number>();
    const colors = ["#10b981", "#3b82f6", "#f59e0b", "#ef4444", "#8b5cf6"];
    let colorIndex = 0;

    filteredTransactions.forEach((row) => {
      const metodo = row.metodo_pago || "N/A";
      paymentMap.set(metodo, (paymentMap.get(metodo) || 0) + row.total);
    });
  ...
  const topProducts = useMemo<ProductData[]>(() => {
    ...
    filteredTransactions.forEach((row) => {
      if (
        !row.product ||
        row.product === "Desconocido" ||
        row.product === "Producto" ||
        row.product === "N/A"
      )
        return;
      const product = row.product;
      productMap.set(product, (productMap.get(product) || 0) + row.total);
    });
  ...
  const kpis = [
    {
      ...
      value: `$${totalSales.toLocaleString("es-CO", { maximumFractionDigits: 0 })}`,
      trend: 4.5,
      icon: DollarSign,
    ...
    },
    {
      ...
      value: `${totalUnits.toLocaleString()} uds`,
      trend: 2.1,
      icon: Package,
    ...
    },
    {
      ...
      value: `$${avgMargin.toLocaleString("es-CO", { maximumFractionDigits: 0 })}`,
      trend: -0.5,
      icon: TrendingUp,
      ...
      return (
        ...
        >
          ...
          <h2 className="text-xl font-bold">
            Vista de {activeTab} en construcción
          </h2>
          <p className="text-sm">
            Estamos trabajando en esta sección. Regresa al Dashboard.
          </p>
          <button
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <motion.div transition={{ delay: 0.5 }} className="xl:col-span-2">
            <MonthlySalesChart
              data={monthlySales.length > 0 ? monthlySales : []}
            />
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          ...
          <motion.div transition={{ delay: 0.7 }} className="xl:col-span-1">
            <PaymentMethodChart data={paymentDistribution} />
          </motion.div>
          <motion.div transition={{ delay: 0.8 }} className="xl:col-span-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full lg:w-auto">
              {[
                ...
                },
                {
                  id: "vendedor",
                  label: "Vendedor",
                  options: ["Todos los Vendedores", ...vendorOptions],
                },
                {
                  id: "ciudad",
                  label: "Ciudad",
                  options: ["Todas las Ciudades", ...cityOptions],
                },
                {
                  id: "categoria",
                  label: "Categoría",
                  options: ["Todas las Categorías", ...categoryOptions],
                },
              ].map((filter) => (
                <div key={filter.id} className="flex flex-col min-w-0">
                  <label className="text-[10px] font-bold text-slate-400 uppercase mb-1 ml-0.5 tracking-wider">
                    {filter.label}
                  </label>
                  <select
